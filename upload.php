...
<?php //code for uploading files based on code by W3schools.com
$target_dir = "uploads/";
...
if ($_FILES["fileToUpload"]["error"] == 4){
    echo "<p class='error'> You need to upload a file. </p>";
    $uploadOk = 0;
}
...
if(isset($_POST["submit"])) {
    ...
    if($check !== false) {
        ...
        $uploadOk = 1;
    } else if ($check == false && $_FILES["fileToUpload"]["error"] !== 4){
...
if (file_exists($target_file) && $_FILES["fileToUpload"]["error"] !== 4) {
...
if ($uploadOk == 0 && $_FILES["fileToUpload"]["error"] !== 4) {
    echo "<p class='error'> Sorry, your file was not uploaded. </p>";
    //exit;
// if everything is ok, try to upload file
} else if($uploadOk == 1 && $_FILES["fileToUpload"]["error"] !== 4){
